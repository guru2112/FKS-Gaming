import { NextRequest, NextResponse } from "next/server";
import connectDB from "@/lib/db";
import { createToken } from "@/lib/utils/token";
import { handleApiError } from "@/lib/errorHandler";
import Admin from "@/models/Admin.js";
import User from "@/models/User.js";

export async function POST(req: NextRequest) {
  await connectDB();
  try {
    const { token: googleToken } = await req.json();

    if (!googleToken) {
      return NextResponse.json(
        { success: false, message: "Google token is required." },
        { status: 400 }
      );
    }

    // Fetch user info from Google
    const googleRes = await fetch(
      "https://www.googleapis.com/oauth2/v3/userinfo",
      {
        headers: { Authorization: `Bearer ${googleToken}` },
      }
    );

    if (!googleRes.ok) {
      return NextResponse.json(
        { success: false, message: "Invalid Google token." },
        { status: 401 }
      );
    }

    const googleData = await googleRes.json();
    const { email, name, sub } = googleData;

    const normalizedEmail = email.toLowerCase().trim();

    // Check Admin first
    const admin = await Admin.findOne({ email: normalizedEmail });
    if (admin) {
      const jwtToken = createToken({ _id: String(admin._id), email: admin.email }, "admin");
      return NextResponse.json({
        token: jwtToken,
        role: "admin",
        user: {
          id: String(admin._id),
          name: admin.name,
          email: admin.email,
          avatarUrl: admin.avatarUrl,
          topbarUrl: admin.topbarUrl,
        },
      });
    }

    // Check User
    let user = await User.findOne({ email: normalizedEmail });
    if (!user) {
      // Auto-create user if not found
      user = await User.create({
        name: name || email.split("@")[0],
        email: normalizedEmail,
        passwordHash: sub, // Google sub as placeholder hash
      });
    }

    const jwtToken = createToken({ _id: String(user._id), email: user.email }, "user");
    return NextResponse.json({
      token: jwtToken,
      role: "user",
      user: {
        id: String(user._id),
        name: user.name,
        email: user.email,
        avatarUrl: user.avatarUrl,
        topbarUrl: user.topbarUrl,
      },
    });
  } catch (err) {
    return handleApiError(err);
  }
}
